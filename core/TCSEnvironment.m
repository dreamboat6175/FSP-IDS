...
classdef TCSEnvironment < handle
    ...
    methods (Access = public)
        ...
        function [next_state, reward_def, reward_att, info] = step(obj, defender_deployment, attacker_target)
            %STEP 环境交互主方法（修正版）
            % 
            % 关键修正：环境只负责物理交互，不包含智能体决策逻辑
            % 输入:
            %   defender_deployment - 防御资源部署向量 [1 x n_stations]
            %   attacker_target - 攻击目标站点索引 [标量]
            % 输出:
            %   next_state - 下一状态向量
            %   reward_def - 防御者奖励
            %   reward_att - 攻击者奖励
    ...
    methods (Access = private)
        ...
        function [reward_def, reward_att] = computeRewards(obj, attack_success, damage, attacker_target, defender_deployment, detection_result)
              %COMPUTEREWARDSFIXED 修复后的奖励函数
    ...
    % 1. 防御成功/失败奖励（修正：减少惩罚，增加奖励）
    if attack_success
        defense_reward = -0.5; % 原来-2，现在-0.5，减少惩罚
    else
        defense_reward = 2;    % 原来+1，现在+2，增加正向激励
    end
    ...
    % RADI奖励修正：使用指数衰减，确保正值
    radi_reward = exp(-current_radi * 2); % 0到1之间，RADI越小奖励越大
    ...
    elseif detection_result.is_false_positive
        detection_reward = -0.1; % 减少误报惩罚
    end
    
    % 4. 损害奖励（修正：改为正向奖励）
    damage_reward = (1 - damage) * 0.5; % 损害越小奖励越大
    
    % 5. 资源效率奖励（修正：改为正向）
    total_used = sum(defender_deployment);
    resource_efficiency = 1 - abs(total_used - obj.total_resources) / obj.total_resources;
    resource_efficiency = max(0, resource_efficiency); % 确保非负
    
    % === 修正后的综合奖励（确保大部分情况为正） ===
    reward_def = 0.3 * defense_reward + ...
                 0.2 * radi_reward + ...
    ...
    % 添加基础奖励，确保不会过度负值
    reward_def = reward_def + 0.5; % 基础奖励0.5
end
        ...
        function info = createStepInfo(obj, attack_success, damage, attacker_target, defender_deployment, detection_result, reward_def, reward_att)
            %CREATESTEPINFO 创建步骤信息结构体
            ...
            info.detection_rate = detection_result.detection_prob;
            ...
            info.defender_strategy = obj.defender_strategy;
            info.attacker_strategy = obj.attacker_strategy;
            
            % === 统计信息 ===
            info.recent_success_rate = obj.computeRecentSuccessRate();
            info.recent_detection_rate = obj.computeRecentDetectionRate();
            info.recent_radi_mean = obj.computeRecentRADI();
            
            % === 学习进度信息 ===
            if ~isempty(obj.attacker_Q_table)
                info.attacker_epsilon = obj.attacker_epsilon; % 环境内部探索率
            end
            
            % === 资源利用信息 ===
            info.total_resources_used = sum(defender_deployment);
            info.resource_utilization = sum(defender_deployment) / obj.total_resources;
            
            % === 站点威胁分析 ===
            info.station_threat_levels = obj.attacker_avg_strategy;
            info.station_values = obj.station_values;
        end
        ...
        function detection_rate = computeRecentDetectionRate(obj)
            %COMPUTERECENTIDETECTIONRATE 计算最近检测率
            
            if isempty(obj.detection_history)
                detection_rate = 0;
            else
                window = min(50, length(obj.detection_history));
                detection_rate = mean(obj.detection_history(end-window+1:end));
            end
        end
