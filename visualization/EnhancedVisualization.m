...
classdef EnhancedVisualization < handle
    ...
    methods (Static)
        ...
        function agent_data = extractAgentData(agent, agent_type, config)
            % 从智能体提取数据
            agent_data = struct();
            ...
            % 提取性能指标
            agent_data.performance = EnhancedVisualization.extractPerformanceMetrics(agent, agent_type, config);
        end
        ...
        function performance = extractPerformanceMetrics(agent, agent_type, config)
            % 提取或生成性能指标
            performance = struct();
